...
export const generateLeads = async ({ businessType, location, numberOfLeads, requirements, coordinates }: GenerateLeadsParams): Promise<GenerateLeadsResult> => {
    const model = 'gemini-2.5-flash';
    const prompt = `Generate a detailed list of up to ${numberOfLeads} of the best matching "${businessType}" in ${location}. ${requirements ? `Strictly prioritize businesses that meet these criteria: "${requirements}".` : ''} For each business, provide its name, specific category, full address, phone number, website URL, Google Maps rating (as a number from 1-5), the total count of reviews (as a number), typical opening hours (as a string like "Mon-Fri 9am-5pm"), and its precise latitude and longitude. Return the response as a single, valid JSON array of objects. Each object must have the following keys: "name", "category", "address", "phone", "website", "rating", "reviewsCount", "openingHours", "latitude", "longitude". If a piece of information is not available, use null for that key. Do not include any businesses that do not fit the requested business type.`;
